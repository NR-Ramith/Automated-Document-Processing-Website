...
def remove_noise_and_extract_face(input_image_path, output_image_path, output_size):
    ...
    if len(faces) > 0:
        ...
        roi_x = max(0, x - int(w * 0.4))  # Adjust the expansion factor as needed
        roi_y = max(0, y - int(h * 0.4))  # Adjust the expansion factor as needed
        roi_w = int(w * 1.8)  # Adjust the expansion factor as needed
        ...
        # Resize the main face region to the desired output size
        resized_face = cv2.resize(image[roi_y:roi_y+roi_h, roi_x:roi_x+roi_w], output_size)
...
if __name__ == "__main__":
    print(sys.argv)
    if len(sys.argv) != 6:
        print("Usage: python process_image.py <input_image_path> <output_image_path> <image_width> <image_height> <background_color>")
        sys.exit(1)
    ...
    main_face_size=(int(sys.argv[3]),int(sys.argv[4]))  # Adjust the desired output size as needed
    ...
    background_img.save(output_image_path, format='png')
